...
def generate_launch_description():
    ...
    detection_method = DeclareLaunchArgument(
        "detection_method", default_value="euclidean_clustering"
    )  # or centerpoint
    ...
    ego_filter_param_file = DeclareLaunchArgument(
        "ego_filter_param_file",
        default_value=os.path.join(
            get_package_share_directory("autoware_perception_launch"),
            "param/ego_filter.param.yaml",
        ),
    )
    ...
    # set crop box filter as a component
    ego_cropbox_component = ComposableNode(
        package="pointcloud_preprocessor",
        plugin="pointcloud_preprocessor::CropBoxFilterComponent",
        name="ego_crop_box_filter",
        remappings=[
            ("input", "points_raw/cropped"),
            ("output", "points_raw/ego_cropped"),
        ],
        parameters=[
            LaunchConfiguration("ego_filter_param_file"),
            {
                "input_frame": LaunchConfiguration("tf_output_frame"),
                "output_frame": LaunchConfiguration("tf_output_frame"),
            },
    ...
    # set crop box filter as a component
    inside_polygon_remover_component = ComposableNode(
        package="pointcloud_preprocessor",
        plugin="pointcloud_preprocessor::PolygonRemoverComponent",
        name="polygon_remover_inside",
        remappings=[
            ("polygon_sub", "/perception/lvms_inside_vis"),
            ("input", "points_raw/ego_cropped"),
            # ("input", "points_raw/cropped"),
            ("output", "points_raw/inside_poly_removed"),
        ],
        parameters=[
            {
                "will_visualize": False,
                "remove_outside": False,
                "working_mode": "PolygonSub",
                "polygon_sync_tolerance_sec": 0.2,
            },
        ],
    )
    outside_polygon_remover_component = ComposableNode(
        package="pointcloud_preprocessor",
        plugin="pointcloud_preprocessor::PolygonRemoverComponent",
        name="polygon_remover_outside",
        remappings=[
            ("polygon_sub", "/perception/lvms_outside_vis"),
            ("input", "points_raw/inside_poly_removed"),
            ("output", "points_raw/outside_poly_removed"),
        ],
        parameters=[
            {
                "will_visualize": False,
    ...
    euclidean_cluster_component = ComposableNode(
        ...
        remappings=[
            ("input", "points_raw/outside_poly_removed"),
    ...
    pointcloud_concat_crop_container = ComposableNodeContainer(
        name="pointcloud_concat_crop_container",
        namespace="lidar_detection",
        package="rclcpp_components",
        executable="component_container",
        composable_node_descriptions=[concat_component, cropbox_component, ego_cropbox_component],
        output="screen",
    )

    inside_polygon_remover_container = ComposableNodeContainer(
        name="inside_polygon_remover_container",
        namespace="lidar_detection",
        package="rclcpp_components",
        executable="component_container",
        composable_node_descriptions=[inside_polygon_remover_component],
        output="screen",
    )

    outside_polygon_remover_container = ComposableNodeContainer(
        name="outside_polygon_remover_container",
        namespace="lidar_detection",
        package="rclcpp_components",
        executable="component_container",
        composable_node_descriptions=[outside_polygon_remover_component],
        output="screen",
    )

    # set container to run all required components in the same process
    euclidean_clustering_container = ComposableNodeContainer(
        name="euclidean_clustering_container",
        namespace="lidar_detection",
        package="rclcpp_components",
        executable="component_container",
    ...
    return launch.LaunchDescription(
        [
            ...
            param_file,
            ego_filter_param_file,
            pointcloud_concat_crop_container,
            inside_polygon_remover_container,
            outside_polygon_remover_container,
            euclidean_clustering_container,
